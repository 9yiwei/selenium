...
class TestExceptions:
    ...
    def test_no_such_element_exception(self, driver):
        # 開啟頁面
        exceptions = ExceptionsPage(driver)
        exceptions.open()
        # 點擊新增按鈕
        exceptions.add_btn()
        # 驗證第 2 行輸入欄位是否顯示
        exceptions.row2_is_displayed()
    ...
    def test_element_not_interactable_exception(self, driver):
        ...
        exceptions.row2_type("Curry")
        # 驗證文字已儲存
        assert exceptions.get_confirmation_text() == "Row 2 was saved", "Row 2 text should be display, but it's not"
    ...
    def test_invalid_element_state_exception(self, driver):
        ...
        exceptions.open()
        # 清除輸入字段
        exceptions.row1_edit()
        # 在輸入欄位中輸入文字
        exceptions.row1_type("Curry")
        # 驗證文字已更改
        assert exceptions.get_confirmation_text() == "Row 1 was saved", "Row 2 text should be display, but it's not"
    ...
    def test_tale_element_reference_exception(self, driver):
        ...
        exceptions.open()
        # 尋找說明文字元素
        assert exceptions.instructions_is_displayed(), "dir should be display, but it's not."
        # 按下新增按鈕
        exceptions.add_btn()
        # 驗證說明文字元素不再顯示
        assert not exceptions.instructions_is_displayed(), "dir should not be display."
    ...
    def test_timeout_exception(self, driver):
        ...
        # 驗證第二個輸入欄位是否顯示
        assert exceptions.row2_is_displayed()
